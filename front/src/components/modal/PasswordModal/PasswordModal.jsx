...
const PasswordModalForm = Form.create({ name: 'form_in_modal' })(
  ...
  class extends Component {
    render() {
      ...
      return (
        <Modal visible={visible} title={mode !== 'decrypt' ? 'Encryption' : 'Decryption'} okText={mode !== 'decrypt' ? 'Encrypt' : 'Decrypt'} onCancel={onCancel} onOk={onCreate}>
              ...
              <Form.Item label='Hint'>
                {mode !== 'decrypt'
                  ? getFieldDecorator('hint', {
                    ...
                    })(<Input placeholder='Input Hint' prefix={<Icon type='info-circle' style={{ color: 'rgb(0,0,0)' }} />} />)
                  : hint}
              </Form.Item>
            ) : null}
            {mode !== 'decrypt' && (
              <Form.Item label='Enter Email you want to share this file with'>
...
class PasswordModal extends Component {
  ...
  render() {
    return (
      <PasswordModalForm wrappedComponentRef={this.saveFormRef} visible={this.props.modal.show} onCancel={this.onReset} onCreate={this.handleCreate} hint={this.props.hint} mode={this.props.mode} />
    );
  }
}
