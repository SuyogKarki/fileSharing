...
export const initialize = props => {
  ...
  if (props.files.mode !== 'decrypt') {
    ...
    steps = [
      ...
      'Sending File to the email address',
    ];
  } else {
    steps = ['Salt and Hash Key', 'Read Cryptionfile', 'Decrypt File with AES', 'Generate SHA256-Hash', 'Check Data Integrity', 'Decrypt Filename', 'Recreate File'];
  }
  ...
  if (props.files.mode === 'encrypt-multiple') {
    worker.combineToZip(props.files.fileList).then(data => {
      props.nextStep();
      worker.encrypt(data, 'package.zip', props.files.password, props.files.hint, props.files.email, props.files.id).then(obj => {
        Modal.success({
          title: 'Success',
          content: 'The files were encrypted successfully and sent to the user',
          onOk: () => props.reset(),
        });
    ...
    worker.fileToData(props.files.fileList[0]).then(data => {
      props.nextStep();
      worker.encrypt(data, props.files.fileList[0].name, props.files.password, props.files.hint, props.files.email, props.files.id).then(() => {
        Modal.success({
          title: 'Success',
          content: 'The file was encrypted successfully and sent to the user',
          onOk: () => props.reset(),
        });
